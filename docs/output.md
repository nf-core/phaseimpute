...
- `imputation/[glimpse1,glimpse2,quilt,stitch]/`
  - `all.batch*.vcf.gz`: The concatenate vcf of all imputed sample.
  - `all.batch*.vcf.gz.tbi`: A tbi for the concatenated samples imputed vcf.
  - `*.vcf.gz`: A vcf of each imputed sample.
  - `*.vcf.gz.tbi`: A tbi for the imputed vcf.
